    ...
    on run argv -- argv is a list of strings
        tell application "Terminal"
            ...
            if $choice is "Icon" then
              do script ("python src/runtrayicon.py") in T
            else if $choice is "Language" then
              do script ("python src/taskslocales.py") in T
            else
              do script ("python src/tasks.py $choice") in T
            end
            repeat
              delay 1
